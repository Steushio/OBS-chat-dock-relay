...
class StreamManager {
  constructor() {
    ...
    this.ignoredVideoIds = new Set(); // Video IDs to ignore on refresh
  ...
  startMonitoring() {
    ...
    this.monitorWindow.webContents.setAudioMuted(true);
    ...
    const MONKEYPATCH_CODE = `
      (function() {
        ...
        // 3. DOM scraper fallback — runs every 20s to find video links on the page
        setInterval(function() {
          if (window.__obsChatStreamActive) return;
        ...
        (function startSubscriberPoller() {
          ...
          setInterval(pollSubscribers, 60000);
    ...
    this.startReloadSync();
  }

  startReloadSync() {
    if (!this.isMonitoring) return;
    this.stopReloadSync();
    websocketServer.logSystem('[Monitor] Periodic background sync check started.');
  ...
  stopReloadSync() {
    if (this.reloadInterval) {
      ...
      websocketServer.logSystem('[Monitor] Background sync reload paused (Stream is active).');
    }
  }
  ...
  addOrUpdateStream(streamInfo) {
    ...
    // Ignore if user manually purged this video ID (unless the API explicitly reports it as active live)
    if (this.ignoredVideoIds.has(streamInfo.videoId) && streamInfo.liveState !== 'live') {
      return;
    }
  ...
  evaluateActiveStream() {
    ...
    const isLive = bestStream.liveState === 'live';
    if (isLive) {
      this.stopReloadSync();
    } else {
      this.startReloadSync();
    ...
    if (this.monitorWindow && !this.monitorWindow.isDestroyed()) {
      this.monitorWindow.webContents.executeJavaScript(`window.__obsChatStreamActive = ${isLive};`).catch(() => {});
    }
  ...
  clearState() {
    if (this.activeStream && this.activeStream.videoId) {
      this.ignoredVideoIds.add(this.activeStream.videoId);
      websocketServer.logSystem(`[Monitor] Purged old stream ID: ${this.activeStream.videoId} from detection.`);
    }
    this.streamsMap.clear();
    this.activeStream = null;
    websocketServer.updateStreamState(null);
    websocketServer.clearLogsAndChats();

    if (this.monitorWindow && !this.monitorWindow.isDestroyed()) {
      this.monitorWindow.webContents.executeJavaScript(`window.__obsChatStreamActive = false;`).catch(() => {});
      // Immediately reload to trigger instant stream search bypass delay
      this.monitorWindow.webContents.reload();
    }
    this.startReloadSync();
  }
